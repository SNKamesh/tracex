...
const BANNED_WORDS = ["fuck", "shit", "bitch", "nigga", "nigger", "porn"];
...
export default function SignupClient() {
  ...
  async function handleSignIn() {
    ...
    setSiPassErr("");
    if (!siEmail.includes("@")) {
      setSiEmailErr("Enter a valid email.");
      return;
    }
    setSiLoading(true);
    try {
  ...
  async function handleSendOtp() {
    ...
    if (caPass !== caPass2) {
      setCaPassErr("Passwords mismatch.");
      return;
    }
    setCaLoading(true);
    try {
      ...
      if (methods.length > 0) {
        setCaEmailErr("Account exists. Sign in.");
        return;
      }
      ...
      <div className="w-full max-w-lg">
        {step === "start" && (
          <div className="flex flex-col items-center">
            <h1 className="mb-2 text-center text-5xl font-extrabold tracking-tight">
              Welcome to <span style={{ color: "#00d8ff" }}>TraceX</span>
            </h1>
            <p className="mb-12 text-center text-xl font-medium text-slate-400">
            ...
            <div className="flex w-full max-w-sm flex-col gap-4">
              ...
              <p
                className="mt-6 cursor-pointer text-center text-sm text-slate-400 underline transition hover:text-white"
                onClick={() => {
                  setStep("create_form");
                  setCaEmailErr("");
                  setCaPassErr("");
