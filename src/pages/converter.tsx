...
async function pdfText(file: File) { const { getDocument, GlobalWorkerOptions } = await import("pdfjs-dist"); GlobalWorkerOptions.workerSrc = "https://cdnjs.cloudflare.com/ajax/libs/pdf.js/5.7.284/pdf.worker.min.mjs"; const pdf = await getDocument({ data: new Uint8Array(await file.arrayBuffer()) }).promise; const pages: string[] = []; for (let i = 1; i <= pdf.numPages; i++) { const page = await pdf.getPage(i); const content = await page.getTextContent(); pages.push(content.items.map((item: any) => "str" in item ? item.str : "").join(" ")) }; return pages.join("\n\n") }
async function renderPdfPages(file: File, type: "png" | "jpg") { const { getDocument, GlobalWorkerOptions } = await import("pdfjs-dist"); GlobalWorkerOptions.workerSrc = "https://cdnjs.cloudflare.com/ajax/libs/pdf.js/5.7.284/pdf.worker.min.mjs"; const pdf = await getDocument({ data: new Uint8Array(await file.arrayBuffer()) }).promise; const results: Blob[] = []; for (let i = 1; i <= pdf.numPages; i++) { const page = await pdf.getPage(i); const viewport = page.getViewport({ scale: 1.6 }); const canvas = document.createElement("canvas"); canvas.width = viewport.width; canvas.height = viewport.height; const context = canvas.getContext("2d"); if (!context) throw new Error("Canvas rendering is unavailable"); await page.render({ canvas, canvasContext: context, viewport }).promise; const blob = await new Promise<Blob>((resolve, reject) => canvas.toBlob((value) => value ? resolve(value) : reject(new Error("Could not render page")), type === "jpg" ? "image/jpeg" : "image/png", 0.92)); results.push(blob) }; return results }
async function docxText(file: File) { const mammoth = await import("mammoth"); return (await mammoth.extractRawText({ arrayBuffer: await file.arrayBuffer() })).value }
async function makePdf(text: string) { const { PDFDocument, StandardFonts, rgb } = await import("pdf-lib"); const pdf = await PDFDocument.create(); const font = await pdf.embedFont(StandardFonts.Helvetica); const lines = text.split(/\r?\n/); let page = pdf.addPage([595, 842]); let y = 800; for (const line of lines) { const chunks = line.match(/.{1,95}/g) ?? [""]; for (const chunk of chunks) { if (y < 40) { page = pdf.addPage([595, 842]); y = 800 }; page.drawText(chunk, { x: 40, y, size: 11, font, color: rgb(0.12, 0.12, 0.12) }); y -= 16 }; y -= 4 }; return new Blob([await pdf.save()], { type: "application/pdf" }) }
async function imageBlob(file: File, target: string) { const bitmap = await createImageBitmap(file); const canvas = document.createElement("canvas"); canvas.width = bitmap.width; canvas.height = bitmap.height; const ctx = canvas.getContext("2d"); if (!ctx) throw new Error("Image rendering is unavailable"); ctx.drawImage(bitmap, 0, 0); bitmap.close(); const mime = target === "jpg" || target === "jpeg" ? "image/jpeg" : target === "webp" ? "image/webp" : "image/png"; return new Promise<Blob>((resolve, reject) => canvas.toBlob((blob) => blob ? resolve(blob) : reject(new Error("Image conversion failed")), mime, 0.92)) }
async function imagesToPdf(files: File[]) { const { PDFDocument } = await import("pdf-lib"); const pdf = await PDFDocument.create(); for (const file of files) { const bytes = await file.arrayBuffer(); const ext = extensionOf(file.name); const image = ext === "jpg" || ext === "jpeg" ? await pdf.embedJpg(bytes) : await pdf.embedPng(bytes); const scale = Math.min(555 / image.width, 780 / image.height, 1); const page = pdf.addPage([595, 842]); page.drawImage(image, { x: (595 - image.width * scale) / 2, y: (842 - image.height * scale) / 2, width: image.width * scale, height: image.height * scale }) }; return new Blob([await pdf.save()], { type: "application/pdf" }) }
function parseDelimited(text: string, delimiter: string) { const rows = text.trim().split(/\r?\n/).map((line) => line.split(delimiter)); const headers = rows[0] ?? []; return rows.slice(1).map((row) => Object.fromEntries(headers.map((key, i) => [key, row[i] ?? ""]))) }
function jsonToCsv(value: any[]) { if (!Array.isArray(value) || !value.length) return ""; const keys = [...new Set(value.flatMap((row) => Object.keys(row)))]; const quote = (v: any) => `"${String(v ?? "").replace(/"/g, '""')}"`; return [keys.map(quote).join(","), ...value.map((row) => keys.map((key) => quote(row[key])).join(","))].join("\n") }
...
async function convertFile(file: File, from: string, to: string): Promise<Result> {
  ...
  if (from === "pdf" && (to === "png" || to === "jpg")) { const pages = await renderPdfPages(file, to); if (pages.length === 1) return { name: file.name.replace(/\.pdf$/i, `.${to}`), blob: pages[0] }; const packed = zipStore(await Promise.all(pages.map(async (blob, i) => ({ name: `page-${String(i + 1).padStart(3, "0")}.${to}`, data: new Uint8Array(await blob.arrayBuffer()) })))); return { name: file.name.replace(/\.pdf$/i, "-pages.zip"), blob: packed } }
  ...
  if (["txt", "html", "htm", "md"].includes(from) && ["txt", "html", "htm", "md"].includes(to)) { const text = await file.text(); const output = to === "html" || to === "htm" ? `<html><body><pre>${escapeXml(text)}</pre></body></html>` : text.replace(/<[^>]*>/g, ""); return { name: file.name.replace(/\.[^.]+$/i, `.${to}`), blob: new Blob([output], { type: to === "html" || to === "htm" ? "text/html" : "text/plain" }) }
  ...
  if (["jpg", "jpeg", "png", "webp"].includes(from)) { if (to === "pdf") return { name: file.name.replace(/\.[^.]+$/i, ".pdf"), blob: await imagesToPdf([file]) }; if (["jpg", "jpeg", "png", "webp"].includes(to)) return { name: file.name.replace(/\.[^.]+$/i, `.${to}`), blob: await imageBlob(file, to) } }
